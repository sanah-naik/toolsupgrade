...
async function mergeApacheConfig(oldConfPath, newConfPath, outputPath, logs, sessionId) {
  ...
  const oldExists = await fs.pathExists(oldConfPath);
  const newExists = await fs.pathExists(newConfPath);

  if (!oldExists) {
      logs.push(`[APACHE_MERGE] ⚠ Old config not found: ${oldConfPath} - skipping`);
      return { merged: false, reason: 'old_not_found' };
  }

  if (!newExists) {
      logs.push(`[APACHE_MERGE] ⚠ New config not found: ${newConfPath} - this is a custom file`);
      logs.push(`[APACHE_MERGE] → Will copy from old installation instead of merging`);
      await fs.copy(oldConfPath, outputPath, { overwrite: true });
      return { merged: false, reason: 'custom_file_copied', userSettings: 1 };
  }
...
async function smartMergeConfiguration(
  toolType,
  oldConfigPath,
  newConfigPath,
  outputPath,
